...
- **Save to:** `sites/roberts-mobile-services/photos-generated/hero-postcard.jpg`
...
Multi-tool mobile service van with side door open, equipment on right side organized.

Bruce can refine wording, but every prompt MUST include the composition requirements above.

## Output

1. `sites/roberts-mobile-services/photos-generated/hero-postcard.jpg` (3360×1872 JPEG)
2. `sites/roberts-mobile-services/bruce-asset-intel.json` with new `generated_images[]` entry (purpose=postcard-hero, prompt, model=gpt-image-2, license_note, intended_slot_context, width=3360, height=1872). Set `model_stack.image_generation` = `openai/gpt-image-2`.

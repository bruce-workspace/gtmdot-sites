...
# Collect-request — done-right-drywall (swap site hero to use postcard hero)
...
1. **Copy** `sites/done-right-drywall/photos-generated/hero-postcard.jpg` (3840×2160 or 3360×1872, gpt-image-2) over the current `photos/hero.jpg` in whatever source the `done-right-drywall.pages.dev` deploy uses
2. **Rebuild and redeploy** the prospect site
3. **Verify** `https://done-right-drywall.pages.dev/photos/hero.jpg` returns the new hero (size should match `https://gtmdot-postcards.pages.dev/done-right-drywall-hero.jpg`)

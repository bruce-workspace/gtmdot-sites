...
site_built
  ...
  ↓ (Mini routes to one of three based on capture state + reputation)
needs_enrichment ──┐  (Bruce/R1VS owes work — automation lane)
needs_decision  ──┤   (Jesse judgment call — borderline rep / dormant / unclear)
needs_approval  ──┘   (clean, awaiting Jesse's approve click)
  ↓ (Jesse reviews live preview on mobile + clicks approve)
qa_approved
...
| `site_built → needs_*` | Claim bar injected + deployed + Supabase note written + Mini routes by capture/reputation state | Mini (auto) |
| `needs_decision → needs_approval` | Jesse decides "yes ship this" | **Jesse (manual)** |
...
*Last updated: 2026-04-25T02:45:28Z*

...
*Placeholder — to be populated by `scripts/state-snapshot.sh` on a post-commit hook (not yet built).*

Until automation is in place, check manually:

```sql
-- Stage counts
SELECT stage, COUNT(*) FROM prospects WHERE disqualified=false GROUP BY stage;

-- Open feedback flags
SELECT feedback_status, COUNT(*) FROM notes
WHERE note_type='design_feedback' AND feedback_status IN ('open','in_progress')
GROUP BY feedback_status;

-- In-flight Bruce requests
-- (check filesystem): find sites -name collect-request.md | wc -l

-- Fresh Bruce deliveries awaiting Mini integration
-- find sites -name bruce-collected.md -mtime -2 | wc -l
```

**Last manually-captured state (2026-04-23 evening):**
- `ready_for_review`: 30 (after 3 DQ'd this morning)
- `outreach_staged`: 14
- `outreach_sent`: 3
- `dead`: 14
- Open design_feedback: 3 (all from Mini's 2026-04-22 overnight pass — see `briefs/` for disposition)
